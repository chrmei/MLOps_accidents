    ...
    try:
        ...
        if mlflow_enabled:
            ...
            try:
                ...
                features_path = paths_config.get(
                ...
                if not os.path.exists(interim_path):
                    interim_path = paths_config.get(
                        "interim", "data/preprocessed/interim_dataset.csv"
                    )

                if os.path.exists(interim_path):
                    logger.info(
                        f"Loading interim dataset from {interim_path} for MLflow signature"
                    )
                    ...
                    logger.info(
                        f"Interim dataset loaded and split: {X_train_interim.shape}"
                    )
                else:
                    logger.warning(
                ...
                logger.warning(
                    f"Failed to load interim dataset for MLflow signature: {e}"
                )
